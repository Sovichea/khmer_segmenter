...
pub fn is_separator_cp(cp: char) -> bool {
    ...
    if (cp as u32) < 0x80 && (cp.is_ascii_punctuation() || cp.is_ascii_whitespace()) {
        return true;
    }
    ...
    if cp == '\u{00AB}' || cp == '\u{00BB}' {
        return true;
    }

    // General Punctuation (0x2000-0x206F)
    if cp >= '\u{2000}' && cp <= '\u{206F}' {
        return true;
    }

    // Currency Symbols (0x20A0-0x20CF)
    if cp >= '\u{20A0}' && cp <= '\u{20CF}' {
        return true;
    }

    // Latin-1 Currency
    if cp == '\u{00A3}' || cp == '\u{00A5}' {
        return true;
    }
...
pub fn is_valid_single_base_char(cp: char) -> bool {
    ...
    if cp >= '\u{1780}' && cp <= '\u{17A2}' {
        return true;
    }
    // Independent Vowels: 0x17A3 - 0x17B3
    if cp >= '\u{17A3}' && cp <= '\u{17B3}' {
        return true;
    }
    false
}
...
pub fn get_khmer_cluster_length(text: &str) -> usize {
    ...
    while let Some(next_cp) = iter.next() {
        ...
        if (next_cp >= '\u{17B6}' && next_cp <= '\u{17D1}')
            || next_cp == '\u{17D3}'
            || next_cp == '\u{17DD}'
        {
