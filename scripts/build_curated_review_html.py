...
function unresolvedCard(row){const classes=['name','valid_missing_term','typo','noise'];return `<article class="card"><div class="card-head"><div class="badges"><span class="badge">${esc(row.case_id)}</span><span class="badge">${esc(row.model_source)}</span><span class="badge">${esc(row.category)}</span></div><span class="muted">${esc(row.sentence_id)}</span></div><span class="label">Sentence context</span><div class="sentence kh">${highlighted(row.source_text,+row.span_start,+row.span_end)}</div><span class="label">Classification</span><div class="choices">${classes.map((v,i)=>`<button class="btn choice ${row.reviewer_classification===v?'selected':''}" data-action="${v}"><small>${i+1}</small>${v.replaceAll('_',' ')}</button>`).join('')}</div><label class="label">Reviewed boundaries inside this span</label><input id="reviewedText" class="wide-input kh" value="${esc(row.reviewed_segmented_text)}"><label class="label">Suggested RAC entry, when applicable</label><input id="suggestedEntry" class="wide-input kh" value="${esc(row.suggested_rac_entry)}"><label class="label">Reviewer notes</label><textarea id="notes" rows="3">${esc(row.reviewer_notes)}</textarea></article>`}
